...
public class PraseContent {
	...
	public PraseContent(MimeMessage mesg) {
		this.mesg = mesg;
	}
	...
	private void praseMulti(MimeMultipart mp) {
		try {
			int n = mp.getCount();
			for (int i = 0; i < n; i++) {
	...
	private void praseBody(MimeBodyPart mbp) {
		try {
	...
	public static PraseContent getInstance(MimeMessage mesg) {
		return new PraseContent(mesg);
	}
}
